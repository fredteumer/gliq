...
In addition to the deterministic, algorithmic decision support, an additional report is generated by an AI agent, providing a subject, at-time analysis to be coupled with the final deterministic report. 
...
Full cloud provisioning and deployment: ➡️ [`docs/DEPLOYMENT.md`](docs/DEPLOYMENT.md).

### Dependencies

- **Python 3.11+**
- Per-component extras (each component installs only what it needs, so the three stay independently deployable):
  - `intake` — FastAPI + uvicorn, Pub/Sub, `openai`/`anthropic`, psycopg
  - `scoring` — Pub/Sub, psycopg, pandas
  - `reporting` — Pub/Sub, psycopg, Jinja2, `openai`/`anthropic`
  - `etl` — psycopg, ijson, alembic, pandas (ops only; not on the component VMs)
  - `dev` — pytest, ruff
- **Cloud (for deployment only):** a GCP project, [Pulumi](https://www.pulumi.com/docs/install/), and Tailscale for admin access. Not needed to run the tests.

### Quickstart

```bash
# 1. Create a virtualenv and install a component's extras plus dev tools
python3 -m venv .venv && . .venv/bin/activate
pip install -e '.[intake,dev]'          # swap in scoring / reporting as needed

# 2. Run the test suite — pure functions, no DB, no network, no API key
pytest
ruff check .

# 3. Try extraction offline against a bundled sample
python -c "from components.intake.extract import extract; from pathlib import Path; \
p = extract(Path('samples/strong-pitch.md').read_text()); \
print(p.title, '|', p.primary_genre, '|', [*p.tags])"
```

To exercise the full pipeline (A→B→C) against real infrastructure, or to serve the review UI, follow [`docs/DEPLOYMENT.md`](docs/DEPLOYMENT.md).
...
<sub>🎓 Built for the Johns Hopkins University AI Engineering Certificate — Cloud-Based System Design Individual Project and Capstone Projects.</sub>
